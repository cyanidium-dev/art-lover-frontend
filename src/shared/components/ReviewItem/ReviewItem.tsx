...
const ReviewItem = (props: Props) => {
  return (
    <li key={props.item.id} className="w-[200px] h-[358px] 
    flex flex-col gap-3
 
    rounded-[8px] 
    custom-shadow
    bg-white
    p-[12px] border-1 border-transparent">
      <div className="w-[176px] h-[85px] rounded-md overflow-hidden relative">
        <Image
          src={props.item.img}
          alt={props.item.name}
      ...
      <Rating rating={props.item.rating} className="text-left" fill="var(--main-orange)" stroke="var(--main-orange)" height={20} width={20} />
        ...
        <div className="flex gap-2 items-center ">
          <TextSubtitle text={props.item.name} className="text-left font-normal text-[var(--main-orange)] " fontSize="20px" color={"var(--main-orange)"} />
